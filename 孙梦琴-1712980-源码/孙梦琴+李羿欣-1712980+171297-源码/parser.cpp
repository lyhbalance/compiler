...
void Parser::parserInit() {
	...
	parserGramTree(root);		//开始分析语法树
}

void Parser::parserGramTree(struct gramTree* node) {
	...
	//继续向下分析
	if (node != NULL) {
		parserGramTree(node->left);
		parserGramTree(node->right);
	}
}

struct gramTree* Parser::parser_statement(struct gramTree* node) {
...
void Parser::parser_jump_statement(struct gramTree* node) {
...
void Parser::parser_expression_statement(struct gramTree *node) {
	if (node->left->name == "expression") {
		parser_expression(node->left);
	}
}

varNode Parser::parser_expression(struct gramTree* node) {
	if (node->left->name == "expression") {
...
void Parser::parser_compound_statement(struct gramTree* node) {
	//继续分析处理compound_statement
	parserGramTree(node);
}

//if else
void Parser::parser_selection_statement(struct gramTree* node) {
...
void Parser::parser_iteration_statement(struct gramTree* node) {
...
//函数定义
struct gramTree* Parser::parser_function_definition(struct gramTree* node) {
...
void Parser::parser_parameter_list(struct gramTree* node,string funcName,bool definite) {
...
void Parser::parser_parameter_declaration(struct gramTree* node, string funcName,bool definite) {
...
struct gramTree* Parser::parser_declaration(struct gramTree *node) {
	//cout << "at " << node->name << endl;
	//node = declaration
...
void Parser::parser_init_declarator_list(string vartype, struct gramTree* node) {
...
//分析变量初始化
void Parser::parser_init_declarator(string vartype, struct gramTree* node) {
	//cout << "at " << node->name << endl;
...
varNode Parser::parser_assignment_expression(struct gramTree* assign_exp) {	//返回变量节点
...
varNode Parser::parser_logical_or_expression(struct gramTree* logical_or_exp) {
...
varNode Parser::parser_logical_and_expression(struct gramTree* logical_and_exp) {
...
varNode Parser::parser_inclusive_or_expression(struct gramTree* inclusive_or_exp) {
...
varNode Parser::parser_exclusive_or_expression(struct gramTree *exclusive_or_exp) {
...
varNode Parser::parser_and_expression(struct gramTree* and_exp) {
...
varNode Parser::parser_equality_expression(struct gramTree* equality_exp) {
...
varNode Parser::parser_relational_expression(struct gramTree* relational_exp) {
...
varNode Parser::parser_shift_expression(struct gramTree*shift_exp) {
...
varNode Parser::parser_additive_expression(struct gramTree* additive_exp) {
...
varNode Parser::parser_multiplicative_expression(struct gramTree* mult_exp) {
...
varNode Parser::parser_unary_expression(struct gramTree*unary_exp) {
...
varNode Parser::parser_postfix_expression(struct gramTree* post_exp) {
	//cout << "here" << endl;
...
void Parser::parser_argument_expression_list(struct gramTree* node, string funcName) {
...
varNode Parser::parser_primary_expression(struct gramTree* primary_exp) {
	...
	else if (primary_exp->left->name == "CONSTANT_STRING") {
		string content = primary_exp->left->content;
		string tempname = "temp" + inttostr(innerCode.tempNum);
		++innerCode.tempNum;
		
		varNode newNode = createTempVar(tempname, "string");
		blockStack.back().varMap.insert({ tempname,newNode });
...
int Parser::getBreakBlockNumber() {
	int N = blockStack.size();
	for (int i = N - 1; i >= 0; i--) {
